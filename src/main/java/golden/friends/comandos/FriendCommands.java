...
public class FriendCommands implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (label.equalsIgnoreCase("friend")) {
            if (args.length == 0) {
                sender.sendMessage(Color.translate(prefix + " &aUsa /friend help para obtener ayuda"));
                return true;
                ...
                return new friendsettings(friendManager, prefix).execute(sender);
            } else if (args[0].equalsIgnoreCase("menu")) {
                return new friendmenu(friendManager).execute(sender);
            } else {
                sender.sendMessage(Color.translate(prefix + " &aUsa /friend help para ver la lista de comandos disponibles"));
                return true;
            }
        }
        return false;
